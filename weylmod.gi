...
##
##
#Y  Copyright (C)  2009, 2015  S.R. Doty (modified by A. Hazi)
##
...
function(p, wt, t, r) 
  ...
  W:= Objectify(NewType( FamilyObj(V), IsWeylModule ), 
              rec(prime:=p,highestWeight:=wt,type:=t,rank:=r,LieAlgebra:=L,
                  ...
                  submodules:=[],weights:=[],basis:=[],wtBasisIndex:=[])
              );
  return(W);
end );
...
function(W)
 ...
 if Length(W!.basis)=0 then
   W!.basis:= BasisVectors(Basis(W!.module));
 fi;
 return W!.basis;
end );
...
function(V)
 ...
 if Length(V!.weights) = 0 then
   V!.weights:= DuplicateFreeList(List(BasisVecs(V), Weight));
 fi;
 return( V!.weights );
end );
...
function(V)
 ...
 local i,out,currentwt,wtBasisDict, bb, wts;
 
 bb:= BasisVecs(V);
 wts:= Weights(V);
 if Length(V!.wtBasisIndex)=0 then
   i:=1;
   while i <= Length(bb) do
       ...
       while (Weight( bb[i] ) = currentwt) do
         Add(wtBasisDict, i);
         i:= i+1;
         if i>Length(bb) then break; fi;
       od;
       Add(V!.wtBasisIndex, wtBasisDict);
   od;
 fi;
...
#############################################################################
InstallMethod(SumCharacter, "for two characters", true, 
[IsList,IsList], 0, 
function(ch1,ch2)
 # returns the sum of two characters (new function!)
 local j, new, out;
 out:= [ ];
 for j in [2,4..Length(ch1)] do
   if ch1[j-1] in ch2 then
...
"for a Weyl module, a submodule weight basis, and a given weight vector", true, 
...
function(V,basis,vec)
 ...
 local p, row, wt, rowbasis, v, rowvec, S; 
 
 row:= function(vec, wt)
  # converts <vec> of weight wt to a (compressed) row vector 
  # with mod p coefficients
  local v,e,k,j,out,p,wtBasisDict;
  p:= V!.prime;
  
  out:= 0*(Z(p)^0)*[1..Length(WeightSpace(V, wt))];
  # record labels for basis vectors for weight space
  j:=Position(Weights(V), wt);
  wtBasisDict:= V!.wtBasisIndex[j];
  
  e:= ExtRepOfObj(ExtRepOfObj(vec));
  for k in [2,4..Length(e)] do
     if e[k-1][1] in wtBasisDict then
        j:=Position(wtBasisDict,e[k-1][1]);
        out[j] := e[k]*Z(p)^0;
     fi;
  od;
  return( out );
 ...
 od;
 if Length(rowbasis) = 0 then
   Add(rowbasis, row(0*vec, wt));
 fi;
...
#         return(false); fi;
       if not divPowerVec(xsimple[j],vec,p,k) = zerovec then
         return(false); fi;
     od;
 od;
 return(true);
...
#         return(false); fi;
       if not IsWithin(V,basis,divPowerVec(xsimple[j],vec,p,k)) then
         return(false); fi;
     od;
 od;
 return(true);
...
function(V,wt)
 ...
 local rank,i,j,k,p,height,highestPrimePower,vec,rowvec,finalmatrix,wtspace,
       item,outlist,result,join,row,S,matrix,simpleGens,xyr,xsimple,ysimple,
       u,z,wts,wtbasisdicts,rsimple,newwt;
...
#   end; 
 ...
 simpleGens:= function(L)
   # returns xsimple & ysimple & simple roots
   local rank, noPosRoots, k, g, posRoots, simpleRoots;
   g:= LatticeGeneratorsInUEA( L );
   posRoots:= PositiveRoots(RootSystem(L));
   rank:= Length(CanonicalGenerators(RootSystem(L))[1]);
   noPosRoots:= Length(posRoots);
   xsimple:= [ ]; ysimple:= [ ]; simpleRoots:= [ ];
   for k in [1..rank] do 
     Add(ysimple, g[k]);
     Add(xsimple, g[k+noPosRoots]);
     Add(simpleRoots, posRoots[k]);
   od;
   return( [xsimple, ysimple, simpleRoots] );
 end;
 
 wts:= Weights(V);
 wtspace:= WeightSpace(V,wt);
 wtbasisdicts:=V!.wtBasisIndex;
 p:= V!.prime;
 xyr:= simpleGens(V!.LieAlgebra); xsimple:=xyr[1]; ysimple:=xyr[2];
 rsimple:=xyr[3];
 rank:= Length(xsimple);
 height:= highestPrimePower(p, Sum(V!.highestWeight));
 finalmatrix:= [];
 for i in [1..Length(wts)] do
   matrix:= [];
   for j in [1..Length(wtspace)] do
     Add(matrix, 0*(Z(p)^0)*[1..Length(wtbasisdicts[i])]);
   od;
   Add(finalmatrix,matrix);
 od;
 for j in [1..rank] do 
   for k in [0..height] do
     matrix:= [];
     newwt:= (p^k)*rsimple[j]+wt;
     if newwt in wts then
       i:= Position(wts, newwt);
       for vec in wtspace do
#         rowvec:= row( ActOn(Q, (xsimple[j]^(p^k)/Factorial(p^k)), vec) );
         rowvec:= row(divPowerVec(xsimple[j],vec,p,k), newwt);
         Add(matrix, rowvec);
       od;
       finalmatrix[i]:= matrix;
     fi;
   od;
 od;
 
 z:=IdentityMat(Length(wtspace),Z(p));
 for i in [1..Length(finalmatrix)] do
   u:= NullspaceMatDestructive(finalmatrix[i]);
   z:= SumIntersectionMat(u,z)[2];
   if Length(z) = 0 then break; fi;
 od;
...
#############################################################################
InstallMethod(SubWeylModule, "for a Weyl module and weight vector", true, 
[IsWeylModule,IsLeftAlgebraModuleElement], 0, 
function(W,vec)
 # finds a basis for the submodule of <W> generated by <vec>
 local i,j,k,rowbasis,lbasis,newvec,rowvec,L,row,
       p,zerotensor,extendbasis,highestPrimePower,g,gens,noPosRoots,
       height,comps,wt,fam,wts;
 ...
 row:= function(vec, wt)
  ...
  out:= 0*(Z(p)^0)*[1..Length(WeightSpace(W, wt))];
  # record labels for basis vectors for weight space
  j:=Position(Weights(W), wt);
  wtBasisDict:= W!.wtBasisIndex[j];
...
#       Add(gens, ObjByExtRep(fam,[[j,1],1]));
#    od;
# od;
 ...
 lbasis:=[vec];
 for wt in wts do
#   Add(S, TrivialSubspace(GF(p)^Length(WeightSpace(W,wt))));
   Add(rowbasis,[0*(Z(p)^0)*[1..Length(WeightSpace(W,wt))]]);
 od;
 wt:=Weight(vec);
 i:=Position(wts,wt);
 rowbasis[i]:=[row(vec,wt)];
# S[i]:=VectorSpace(GF(p),rowbasis[i]); 

 extendbasis:= function(v)
   local i, j, k, roots, wt, newwt;
   roots:=PositiveRoots(RootSystem(L));
   wt:=Weight(v);
   for i in [1..2*noPosRoots] do
     for j in [0,1..height] do
       if i<=noPosRoots then
         newwt:= wt-(p^j)*roots[i];
       else
         newwt:= wt+(p^j)*roots[i-noPosRoots];
       fi;
       if newwt in wts then
         k:=Position(wts, newwt);
#         newvec:= (i^v) mod p;
         newvec:= divPowerVec(gens[i],v,p,j);
         rowvec:= row(newvec, newwt);
#         if not (rowvec in S[k]) then
         if SolutionMat(rowbasis[k], rowvec) = fail then
           if not rowbasis[k] = NullMat(1,Length(W!.wtBasisIndex[k]), Z(p)) then
             Add(rowbasis[k],rowvec);
           else rowbasis[k]:=[rowvec]; fi;
#           S[k]:= VectorSpace(GF(p), rowbasis[k]);
           Add( lbasis, newvec );
         fi;
       fi;
     od;
...
InstallMethod(SubWeylModule, "for a Weyl module and list of weight vectors", true, 
[IsWeylModule, IsList], 0, 
function(V,vecs)
 # Returns a basis for the submodule generated by the given weight vectors
...
function(V, submodBasis) 
 ...
 local row,p,rowbases,wt,wts,wtBasisDict,wtBasisDicts,v,rowvec,
 S,SS,W,WW,Q,QQ,h,hh,submodBasisByWt,record,
 bq,bw,cosetreps,w,i,j,result;
 ...
 p:= V!.prime;
 wts:= WeightSpaces(V){[1,3..(Length(WeightSpaces(V))-1)]};
 rowbases:= [];
 wtBasisDicts:= V!.wtBasisIndex;
 submodBasisByWt:=[];
 for wt in wts do
...
#    wtBasisDict:=[];
    Add(submodBasisByWt, []);
#    for v in WeightSpace(V,wt) do
#       Add(wtBasisDict, ExtRepOfObj(ExtRepOfObj(v))[1][1]);
#    od;
#    Add(wtBasisDicts,wtBasisDict);
 od;
 for v in submodBasis do
    wt:= Weight(v);
    i:= Position(wts, wt);
    rowvec := row(v, wt);
    Add(submodBasisByWt[i], v);
    Add(rowbases[i], rowvec);
 od;
 for i in [1..Length(rowbases)] do
   if Length(rowbases[i]) = 0 then 
     Add(rowbases[i], 0*Z(p)^0*[1..Length(wtBasisDicts[i])]);
   fi;
 od;
 SS:=[];
 WW:=[];
 ...
 bw:=BasisVecs(V);
 for i in [1..Length(wts)] do
    S:= VectorSpace(GF(p), rowbases[i]);
    W:= GF(p)^Length(wtBasisDicts[i]);
    Q:= W/S;
    h:= NaturalHomomorphismBySubspace(W, S);
    Add(SS, S);
    Add(WW, W);
    Add(QQ, Q);
    Add(hh, h);
    ...
    for v in bq do
       w:= PreImagesRepresentative(h, v);
       result:= 0*bw[1];
       for j in [1..Length(w)] do
          result:= result + IntFFESymm(w[j])*bw[wtBasisDicts[i][j]];
       od;
       Add(cosetreps[i], result);
    od;
 od;

 record:= Objectify(NewType( FamilyObj(V), IsQuotientWeylModule ), 
                  rec(WeylModule:=V,weights:=wts,
                      wtBasisIndex:=wtBasisDicts,
                      submoduleBasis:=submodBasisByWt,
                      correspondingBasis:=rowbases, bigspaces:=WW, 
                      subspaces:=SS,quotients:=QQ,homomorphisms:=hh, 
...
function(W)
 # returns the generator of <W>
 return BasisVecs(W!.WeylModule)[1];
end );
...
function(V)
 # returns a list of the weight spaces of a given module V
 local out,i,cosetreps,wts,wtspacebasis;
 
 cosetreps:= V!.cosetReps;
 wts:=V!.weights;
 out:=[];
 for i in [1..Length(cosetreps)] do
    wtspacebasis:= cosetreps[i];
    if Length(wtspacebasis) <> 0 then
       Add(out, wts[i]);
       Add(out, wtspacebasis);
    fi;
 od;
 return(out);
...
function(Q,x,v)
 ...
 local h,p,V,wt,wts,rep,vwt,result,row,cosetreps,vv,ans,ansmon,i,j,zerovec,
 famvec,famalg,monlist;

 V:= Q!.WeylModule;
 wts:= Q!.weights;
 cosetreps:=Q!.cosetReps;
 p:= V!.prime;
 ...
 famvec:= FamilyObj(ExtRepOfObj(v));
 famalg:= FamilyObj(ExtRepOfObj(x));
 zerovec:= 0*v;
 ans:= zerovec;
 monlist:=ExtRepOfObj(x);
 
 if v = zerovec then return(zerovec); fi;
 for wt in wts do
    rep:= ExtRepOfObj(ExtRepOfObj(v));
    vwt:= zerovec;
    for i in [2,4..Length(rep)] do
       if rep[i-1][3] = wt then
          vwt:= vwt + ObjByExtRep(famvec,rep{[i-1,i]});
       fi;
    od;
    for i in [2,4..Length(monlist)] do
       ansmon:=ObjByExtRep(famalg,monlist{[i-1,i]})^vwt;
       j:=Position(wts, Weight(ansmon));
       h:= Q!.homomorphisms[j];
       result:= Image(h, row(ansmon mod p, Weight(ansmon)));
#       vv:=PreImagesRepresentative(h, result);
       for i in [1..Length(vv)] do
          ans:= ans + IntFFESymm(result[i])*cosetreps[j][i];
       od;
    od;
 od;
 return(ans);
...
function(Q,wt)
 ...
 local rank,i,j,k,p,height,highestPrimePower,vec,rowvec,finalmatrix,wtspace,
       item,outlist,result,join,row,S,matrix,simpleGens,xyr,xsimple,ysimple,
       u,z,V,quotientDivPowerVec,wts,image,cosetreps,newwt,rsimple;
 ...
 quotientDivPowerVec:= function(Q,gen,vec,newwt,p,k)
  # like divPowerVec, but for a quotient Weyl module, and 
  # a weight vector returning row vector and weight.
  local h,wts,result,vv,i,preimage; # ans, cosetreps;
  
  wts:=Q!.weights;
#  cosetreps:= Q!.cosetReps;
  
  preimage:= divPowerVec(gen,vec,p,k);
  i:= Position(wts, newwt);
  h:= Q!.homomorphisms[i];
  result:= Image(h, row(preimage, newwt));
  return(result);
  
#  vv:=PreImagesRepresentative(h, result);
#  ans:= 0*cosetreps[1][1];
#  for i in [1..Length(vv)] do
#     ans:= ans + IntFFESymm(vv[i])*cosetreps[j][i];
#  od;
#  
#  return(ans);
 end;
...
# end; 
 ...
 for i in [1..Length(wts)] do
   matrix:= [];
   for j in [1..Length(wtspace)] do
     Add(matrix, 0*(Z(p)^0)*[1..Length(cosetreps[i])]);
   od;
   Add(finalmatrix,matrix);
 od;
 for j in [1..rank] do 
   for k in [0..height] do
     ...
     if newwt in wts then
       ...
       for vec in wtspace do
#         rowvec:= row( ActOn(Q, (xsimple[j]^(p^k)/Factorial(p^k)), vec) );
         rowvec:= quotientDivPowerVec(Q,xsimple[j],vec,newwt,p,k);
         Add(matrix, rowvec);
       od;
       finalmatrix[i]:= matrix;
     fi;
 ...
 z:=IdentityMat(Length(wtspace),Z(p));
 for i in [1..Length(finalmatrix)] do
   if Length(finalmatrix[i][1]) > 0 then
     u:= NullspaceMatDestructive(finalmatrix[i]);
     z:= SumIntersectionMat(u,z)[2];
   fi;
   if Length(z) = 0 then break; fi;
...
function(V,gens)
 ...
 for v in gens do
    ...
    od;
 od;
 if Length(outlist) = 0 then Print(V); Print(gens); fi;
 return outlist;
end );
...
#############################################################################
InstallMethod(IsWithin, 
"for a quotient Weyl module, a submodule weight basis, and a given weight vector", 
true, [IsQuotientWeylModule,IsList,IsLeftAlgebraModuleElement], 0, 
function(Q,basis,vec)
 # Returns True if the given <vec> lies within the submodule 
 # spanned by the given <basis> vectors.
 local p, row, rowbases, v, rowvec, S, V, hh, i, wts; 
 ...
 od;
 for v in basis do
    i:= Position(wts, Weight(v));
    rowvec := Image(hh[i], row(v, wts[i]));
    Add(rowbases[i], rowvec);
 od;
 i:= Position(wts, Weight(vec));
 S:= VectorSpace(GF(p), rowbases[i]);
 rowvec:= Image(hh[i], row(vec, wts[i]));
 if rowvec in S then
    return true;
 else
    return false;
...
#############################################################################
InstallMethod(SubWeylModule, "for a quotient Weyl module and weight vector", true, 
...
function(Q,vec)
 ...
 local i,j,k,rowbasis,lbasis,newvec,rowvec,S,L,row,
       p,zerotensor,extendbasis,highestPrimePower,g,gens,noPosRoots,
       height,comps,wt,qwts,wwts,W,hh,corrbasis,SS,WW,QQ,quotientDivPowerVec; # fam;
 ...
 quotientDivPowerVec:= function(Q,gen,vec,newwt,p,k)
  # like divPowerVec, but for a quotient Weyl module, and 
  # a weight vector.
  local h,wts,result,vv,i,j,preimage, ans, cosetreps;
...
#  vv:=PreImagesRepresentative(h, result);
  ...
  for j in [1..Length(result)] do
     ans:= ans + IntFFESymm(result[j])*cosetreps[i][j];
  od;
 ...
 qwts:= Weights(Q);
 wwts:= Weights(W);
 corrbasis:=Q!.correspondingBasis;
 height:= highestPrimePower(p, Sum(wt));
...
#    for j in [1..2*noPosRoots] do
#       Add(gens, gens[j]^(p^i)/Factorial(p^i));
#       Add(gens, ObjByExtRep(fam,[[j,p^i],1]));
#    od;
# od;
...
# od;
 wt:= Weight(vec);
 i:=Position(wwts,wt);
 rowbasis:= StructuralCopy(corrbasis);
 rowvec:= row(vec, wt);
 if SolutionMat(rowbasis[i], rowvec) = fail then
   Add(rowbasis[i], row(vec,wt));
 else return([]); fi;
# WW:= Q!.bigspaces;
# SS:=Q!.subspaces;
# QQ:=Q!.quotients;
# SS[i]:= VectorSpace(GF(p), Concatenation(corrbasis[i],rowbasis[i]));
# QQ[i]:= WW[i]/SS[i];
# hh[i]:= NaturalHomomorphismBySubspace(WW[i],SS[i]);
 ...
 extendbasis:= function(v)
   ...
   for i in [1..2*noPosRoots] do
     for j in [0,1..height] do
       ...
       if newwt in qwts then
         k:=Position(wwts, newwt);
#         newvec:= (i^v) mod p;
         newvec:= quotientDivPowerVec(Q,gens[i],v,newwt,p,j);
#         newvec:= divPowerVec(gens[i],v,p,j);
         rowvec:= row(newvec, newwt);
         if SolutionMat(rowbasis[k], rowvec) = fail then
           if not rowbasis[k] = NullMat(1,Length(W!.wtBasisIndex[k]), Z(p)) then
